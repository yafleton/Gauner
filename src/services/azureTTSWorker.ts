...
export class AzureTTSWorkerService {
  ...
  async getAvailableVoices(apiKey?: string, region?: string): Promise<AzureVoice[]> {
    // If no API key provided, return empty list
    if (!apiKey || !region) {
      console.log('🎤 Worker TTS: No API key provided, returning empty voice list');
      return [];
    }

    try {
      console.log(`🎤 Worker TTS: Fetching voices from Worker API`);
      ...
      if (!response.ok) {
        const errorText = await response.text();
        console.error('❌ Worker Voices Error:', response.status, errorText);
        throw new Error(`Worker voices API failed: ${response.status} - ${errorText}`);
      }
      ...
      console.log(`✅ Worker TTS: Retrieved ${result.voices.length} voices from Azure TTS`);
      return result.voices;

    } catch (error) {
      console.error('❌ Worker TTS Voices Error:', error);
